...
variable "asg_lambda_file-cleanup" {
  description            = "File to use for the ASG Lambda function"
  default                = "./core/main/asg_lambda-clean.zip"
}

resource "aws_lambda_function" "autoscaling_event_update_route53-cleanup" {
  function_name          = "autoscaling_event_update_route53-cleanup"

  filename               = "${var.asg_lambda_file-cleanup}"
  source_code_hash       = "${base64sha256(file("${var.asg_lambda_file-cleanup}"))}"
  handler                = "asg_lambda-clean.handler"
